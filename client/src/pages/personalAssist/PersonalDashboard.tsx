...
export default function PersonalDashboard() {
  ...
  const [activityType, setActivityType] = useState<'task' | 'event' | 'habit' | 'reminder' | 'milestone' | 'deadline' | 'personal'>('event');
  ...
  const fetchRealData = async () => {
    try {
      ...
      const actRes = await axios.get(`${API}/personal/activities?month=${monthStr}`);
  ...
  // Next Up Activity Calculation from Real Database
  ...
  // Weekly Days Array (e.g. 4 visible days matching reference: Mon 16, Tue 17, Wed 18, Thu 19)
  const getWeeklyDays = () => {
    const days = [];
  ...
  // Mandatory Fixed Activity Color Palette System
  const getActivityColorStyles = (type: string) => {
    switch (type) {
      case 'task':
        return 'bg-[#fce7f3] text-[#9d174d] border-l-4 border-[#ec4899] hover:bg-[#fbcfe8]';
      case 'event':
        return 'bg-[#e0f2fe] text-[#0369a1] border-l-4 border-[#0284c7] hover:bg-[#bae6fd]';
      case 'habit':
        return 'bg-[#f3e8ff] text-[#6b21a8] border-l-4 border-[#a855f7] hover:bg-[#e9d5ff]';
      case 'reminder':
        return 'bg-[#fef3c7] text-[#92400e] border-l-4 border-[#f59e0b] hover:bg-[#fde68a]';
      case 'milestone':
        return 'bg-[#e0e7ff] text-[#3730a3] border-l-4 border-[#6366f1] hover:bg-[#c7d2fe]';
      case 'deadline':
        return 'bg-[#ffe4e6] text-[#9f1239] border-l-4 border-[#f43f5e] hover:bg-[#fecdd3]';
      case 'personal':
      default:
        return 'bg-[#ccfbf1] text-[#115e59] border-l-4 border-[#14b8a6] hover:bg-[#99f6e4]';
    }
  };
  ...
  return (
    <div className="space-y-4 font-sans text-slate-800 animate-in fade-in duration-300 pb-12">
      ...
      {/* ── 1-TO-1 REFERENCE MATCHED LAYOUT (LEFT PANEL + MAIN CALENDAR) ─────── */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          <div className="bg-white p-4.5 rounded-3xl border border-slate-200/80 shadow-sm space-y-3">
            <div className="flex justify-between items-center px-1">
              <h2 className="text-sm font-black text-slate-900">{monthName} {year}</h2>
          ...
          {/* 2. Prominent Dark Teal NEXT UP Card */}
              ...
              {nextUpActivity && (
                <div className="flex items-center space-x-1">
                  <button 
                    ...
                    className="w-5 h-5 rounded-full bg-amber-400 text-slate-950 flex items-center justify-center text-[10px] font-black shadow"
                    title="Snooze"
                  >
                    ✕
                  </button>
                  <button 
                    ...
                    className="w-5 h-5 rounded-full bg-teal-800 text-white flex items-center justify-center text-[10px] font-bold"
                    title="Complete"
                  >
                    ✓
                  </button>
            ...
            {nextUpActivity ? (
              <div className="space-y-1.5">
                ...
                <div className="flex items-center space-x-1.5 text-xs text-teal-100 font-medium">
                  <Clock size={13} />
                  <span>{nextUpActivity.start_time} - {nextUpActivity.end_time}</span>
                </div>
              ...
              <div className="py-3 text-center space-y-2">
                <p className="text-xs text-teal-100 font-medium">No upcoming activities</p>
                <button 
                  onClick={() => setIsModalOpen(true)}
                  className="px-3 py-1 bg-amber-400 text-slate-950 font-black rounded-xl text-xs shadow-sm"
                >
                  + Add Activity
                </button>
              </div>
        ...
        <div className="lg:col-span-8 bg-white p-5.5 rounded-3xl border border-slate-200/80 shadow-sm space-y-4 min-h-[620px]">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-150 pb-4">
            ...
            {/* View Controls + Golden Yellow Button */}
            <div className="flex items-center space-x-3">
              <div className="flex items-center bg-slate-100 p-1 rounded-xl text-xs font-bold text-slate-600">
                <button
                  onClick={() => setViewMode('daily')}
                  className={`px-3 py-1 rounded-lg capitalize transition ${viewMode === 'daily' ? 'bg-white text-slate-900 shadow-sm font-black' : 'hover:text-slate-900'}`}
                >
                  Daily
                </button>
                <button
                  onClick={() => setViewMode('weekly')}
                  className={`px-3 py-1 rounded-lg capitalize transition ${viewMode === 'weekly' ? 'bg-white text-slate-900 shadow-sm font-black' : 'hover:text-slate-900'}`}
                >
                  Weekly
                </button>
                <button
                  onClick={() => setViewMode('monthly')}
                  className={`px-3 py-1 rounded-lg capitalize transition ${viewMode === 'monthly' ? 'bg-white text-slate-900 shadow-sm font-black' : 'hover:text-slate-900'}`}
                >
                  Monthly
                </button>
              </div>
          ...
          {viewMode === 'weekly' && (
            <>
              ...
              <div className="grid grid-cols-5 gap-2 text-center border-b border-slate-150 pb-2 pl-14">
                {weeklyDays.map(d => (
                  <div key={d.fullDateStr} className="space-y-0.5">
                    ...
                    <span className={`text-base font-black font-mono block ${d.isSelected ? 'text-teal-700 font-extrabold' : 'text-slate-800'}`}>
                      {d.dateNum}
                    </span>
                  </div>
                ))}
              ...
              <div className="space-y-4 relative pt-1 max-h-[520px] overflow-y-auto">
                          ...
                          >
                            {slotEvents.map(ev => (
                              <div
                                ...
                                className={`p-2.5 rounded-2xl space-y-1 transition shadow-sm mb-1 cursor-pointer hover:scale-[1.02] ${getActivityColorStyles(ev.activity_type)} ${ev.status === 'completed' ? 'opacity-60 line-through' : ''}`}
                              >
          ...
          {/* ── DAILY VIEW WORKSPACE ───────────────────────────────────────── */}
          {viewMode === 'daily' && (
            <div className="space-y-3 pt-2">
              <div className="flex justify-between items-center text-xs font-bold text-slate-500 border-b border-slate-150 pb-2">
                <span>Daily Schedule for {selectedDateStr}</span>
                <span>{filteredActivities.filter(a => a.date === selectedDateStr).length} Items</span>
              </div>
                ...
                {filteredActivities.filter(a => a.date === selectedDateStr).length === 0 ? (
                  ...
                  filteredActivities.filter(a => a.date === selectedDateStr).map(act => (
                    <div
                      ...
                      className={`p-3.5 rounded-2xl border transition shadow-sm cursor-pointer flex items-center justify-between gap-3 ${getActivityColorStyles(act.activity_type)}`}
                    >
                      <div className="flex items-center space-x-3">
                        <button 
                      ...
                      <div className="space-y-1">
                        {dayActivities.slice(0, 2).map(act => (
                          <div 
                            ...
                            className={`text-[9px] font-bold px-1.5 py-0.5 rounded truncate ${getActivityColorStyles(act.activity_type)}`}
                          >
                            {act.title}
                          </div>
                        ))}
      ...
      {/* ── CREATE ACTIVITY UNIVERSAL MODAL ──────────────────────────────── */}
      {isModalOpen && (
            ...
            <form onSubmit={handleCreateActivity} className="space-y-3 text-xs">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-1.5 bg-slate-100 p-1 rounded-2xl text-[11px] font-bold">
                  {(['task', 'event', 'habit', 'reminder', 'milestone', 'deadline', 'personal'] as const).map(t => (
                    <button
                      key={t}
                      type="button"
      ...
      {/* ── ACTIVITY DETAILS & BI-DIRECTIONAL ACTIONS MODAL ─────────────── */}
      {selectedActivity && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
